...
def calculate_curvature_offset(self):
    ...
    if self.detected_first:
        # define y value near the runner
        ...
        if not self.poly_warning:
            ...
            midpoint_runner = self.im_shape[1]/2.0
            ...
            offset = midpoint_runner*xm_per_pix - midpoint_lane/2
...
def project_on_road_debug(self, image_input):
    ...
    if self.detected_first & self.detected:
        ...
        if self.offset< -0.4:
            arduinoSerialData.write(str.encode('1'))
            img_text2 = "runner is: " + str(round(np.abs(self.offset),2)) + ' (m) left of center'

        elif self.offset> 0.4:
            arduinoSerialData.write(str.encode('0'))
            img_text2 = "runner is: " + str(round(np.abs(self.offset),2)) + ' (m) right of center'


        else:
            arduinoSerialData.write(str.encode('3'))
            img_text2 = "runner is: " + str(round(np.abs(self.offset),2)) + ' (m) middle of center'
...
def project_on_road(self, image_input):
    ...
    if self.detected_first & self.detected:
        ...
        if self.offset< 0:
            # img_text2 = "runner isa: " + str(round(np.abs(self.offset),2)) + ' (m) left of center'
            img_text2 = "move to right"
        else:
            # img_text2 = "runner isb: " + str(round(np.abs(self.offset),2)) + ' (m) right of center'
            img_text2 = "move to left"
